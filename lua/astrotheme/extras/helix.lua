...
function M.generate(colors, highlights)
  ...
  local mapping = M.flatten {
    ...
    ui = {
      ...
      menu = {
        ...
        scroll = {
          "helix",
          fg = vim.tbl_get(highlights, "PmenuThumb", "bg"),
          bg = vim.tbl_get(highlights, "PmenuSbar", "bg"),
        },
      },
      selection = {
  ...
  for hx_scope, group in M.pairsByKeys(mapping) do
    ...
    if hx_scope:match "%." then hx_scope = '"' .. hx_scope .. '"' end
    ...
    if group then
      if type(group) == "table" and group[1] == "helix" then
        ...
        while highlight and (type(highlight) == "string" or highlight.link) do
          highlight = highlights[type(highlight) == "string" and highlight or highlight.link]
        end
        if highlight then
...
function M.to_helix_config(highlight)
  ...
  for _, mods in ipairs { highlight, highlight.cterm } do
    if mods then
      if mods.bold then modifiers.bold = true end
      if mods.italic then modifiers.italic = true end
      if mods.underline then style.underline = {
        style = "line",
      } end
  ...
  if next(modifiers) ~= nil then style.modifiers = M.key_set(modifiers) end
